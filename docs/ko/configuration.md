...
- **값은 단일 명령 이름이어야 합니다.** 레지스트리 값은 단어 분할 없이 하나의 인자로 tmux 에 전달되므로, `claude --model x` 처럼 공백이 포함된 다중 토큰 명령은 v1.0.0 에서 지원하지 않습니다. 옵션을 붙이고 싶다면 래퍼 스크립트를 만들어 그 이름을 레지스트리에 등록하세요.

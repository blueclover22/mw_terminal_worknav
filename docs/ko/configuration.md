...
- **예약어를 키로 쓸 수 없습니다** — `list` · `new` · `rm` · `help` · `cd`. 고정 명령(`mtw_list` · `mtw_new` · `mtw_rm` · `mtw_help` · `mtw_cd_*`)과 충돌하기 때문입니다.
  **대소문자는 구분하지 않습니다** — `List` · `RM` 도 예약어로 걸러집니다. PowerShell 이 함수 이름의 대소문자를 구분하지 않아 `mtw_List` 가 고정 명령 `mtw_list` 를 덮어쓰기 때문이며, 양 OS 가 같은 키를 같게 거르도록 zsh 판도 같은 규칙을 씁니다.
...
- **값은 단일 명령 이름이어야 합니다.** 레지스트리 값은 단어 분할 없이 하나의 인자로 tmux 에 전달되므로, `claude --model x` 처럼 공백이 포함된 다중 토큰 명령은 v2.0.2 에서 지원하지 않습니다. 옵션을 붙이고 싶다면 래퍼 스크립트를 만들어 그 이름을 레지스트리에 등록하세요.
